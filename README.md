...
IP-сертификаты Let's Encrypt являются short-lived: они действуют примерно 6 дней. Скрипт включает автоматическое продление дважды в день, поэтому сервер должен оставаться включенным, а порты `80` и `443` должны быть открыты.
...
- включает автоматическое продление сертификата через systemd timer;
- создает случайный секретный URL-путь;
- настраивает HTTPS-прокси к Anthropic API;
- ставит nginx-таймауты для долгих Claude Code сессий;
...
Проверить автообновление на сервере:

```bash
systemctl list-timers claude-proxy-certbot-renew.timer
sudo systemctl status claude-proxy-certbot-renew.timer
```
